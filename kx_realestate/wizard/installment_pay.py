...
class InstallmentPaymentCheck(models.TransientModel):
    ...
    def default_get(self, fields):
        result = super(InstallmentPaymentCheck, self).default_get(fields)
        ...
        if model == 'loan.line.rs.rent':
            installment_id = self.env.context.get('active_id')
            installment = self.env['loan.line.rs.rent'].sudo().browse(installment_id)
            result['amount'] = installment.amount_residual or installment.amount
            result['communication'] = (installment.loan_id.name + " - " + installment.name)
        return result
    ...
    def pay(self):
        ...
        if model == 'loan.line.rs.own':
            installment = self.env['loan.line.rs.own'].sudo().browse(installment_id)
            payments = installment.register_customer_payment_via_invoice(
                amount=self.amount,
        ...
        if model == 'loan.line.rs.rent':
            ...
            vals['loan_line_rs_rent_id']= installment.id
