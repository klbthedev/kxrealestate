...
class RealEstateBuildingStage(models.Model):
    _name = "re.building.stage"
    _description = "Building Progress Stage"
    _order = "sequence, id"
    ...
    _sql_constraints = [
        (
            'building_stage_code_type_uniq',
            'unique(code, building_type_id)',
            'Building stage code must be unique per building type.',
        ),
    ]
