...
class Parser(models.AbstractModel):
    _name = 'report.kx_realestate.report_occupancy'

    def _get_units_status(self, building_ids, unit_ids):
        units = []
        result=[]
        ...
        # if region_ids:
        #     building_ids = self.env['building.building'].search([('region_id','in', region_ids)])
        #     buildings=[]
        ...
        for unit in units:
            ...
            unit_line = {}
            unit_line['city']=None
            # unit_line['region_id']=None
            unit_line['building_id']=None
            unit_line['type']=None
            # unit_line['region_id'] = unit_obj.region_id.name
            unit_line['building_id'] = unit_obj.building_id.name
            unit_line['state'] = unit_obj.state
            unit_line['name'] = unit_obj.name
            if  unit_obj.state != 'free':
                rental_ids = rental_pool.search( [('building_unit_id', '=', unit)])
                rentals=[]
                ...
                if len(rentals) > 0:
                    for rental in rentals:
                        dates=[]
                        for line in rental.loan_line_rs_rent_ids:
    ...
    def _get_report_values(self, docids, data=None):
        ...
        occupancy = self.env['ir.actions.report']._get_report_from_name('kx_realestate.report_occupancy')
        return {
            'doc_ids': self.ids,
